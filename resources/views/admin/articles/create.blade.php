...
@section('breadcrumb')
    <a href="{{ route('admin.articles.index') }}">{{ __('Articles') }}</a> / {{ __('Create') }}
@endsection
...
>
    ...
    <div class="flex flex-wrap gap-3 border-t border-secondary/30 pt-8">
        <button type="submit" class="rounded-xl bg-primary px-6 py-3 font-semibold text-white shadow-sm ring-1 ring-black/[0.04] hover:bg-primary/90">{{ __('Save article') }}</button>
