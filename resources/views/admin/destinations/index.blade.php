...
<div class="mx-auto max-w-7xl space-y-10 pb-24">
        ...
        <div class="border-b border-secondary/40 bg-primary/[0.04] px-5 py-4 sm:px-8 sm:py-5">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div class="min-w-0 max-w-3xl">
                    ...
                    <h2 class="mt-1 font-serif text-xl font-semibold text-ink sm:text-2xl">{{ __('Destination listings for visitors') }}</h2>
                    <p class="mt-2 text-sm leading-relaxed text-ink/70">
                        {{ __('Places you manage here appear on Explore Africa, the homepage marquee, and each destination detail page.') }}
                        <a href="{{ route('explore-africa') }}" class="ml-1 inline-flex items-center gap-1 font-semibold text-primary underline decoration-primary/30 underline-offset-2 hover:decoration-primary" target="_blank" rel="noopener noreferrer">
                            {{ __('Open public page') }}
    ...
    <section class="space-y-5" aria-labelledby="destinations-admin-heading">
        <div class="flex flex-col gap-4 border-b border-secondary/40 pb-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <form method="get" class="flex flex-wrap items-center gap-2" role="search">
                    <label class="sr-only" for="destinations-search-q">{{ __('Search') }}</label>
                    <input
                        id="destinations-search-q"
                        name="q"
                        value="{{ $q }}"
                    ...
                    >
                    @if(filled($q))
                        <a href="{{ route('admin.destinations.index') }}" class="rounded-xl border border-secondary/60 px-3 py-2 text-sm font-medium text-ink/70 hover:bg-surface">{{ __('Clear') }}</a>
                    @endif
                ...
                <a href="{{ route('admin.destinations.create') }}" class="inline-flex items-center justify-center rounded-xl bg-accent px-4 py-2.5 text-sm font-semibold text-ink shadow-sm ring-1 ring-black/[0.04] hover:bg-accent/90">
                    {{ __('Add destination') }}
                </a>
            </div>
        </div>
                ...
                <article class="group overflow-hidden rounded-xl border border-secondary/50 bg-white shadow-soft transition hover:border-primary/25 hover:shadow-md">
                    <div class="relative aspect-[4/3] overflow-hidden bg-neutral-100">
                        <img
                            ...
                            height="300"
                        >
                        <span class="absolute left-2 top-2 rounded-md px-2 py-0.5 text-[0.65rem] font-semibold uppercase tracking-wide shadow-sm {{ $d->is_active ? 'bg-primary text-white' : 'bg-ink/70 text-white' }}">
                            {{ $d->is_active ? __('Visible') : __('Hidden') }}
                        </span>
                    </div>
                    <div class="border-t border-secondary/40 p-3">
                        <p class="truncate font-semibold text-ink">{{ $d->name }}</p>
                        <p class="mt-0.5 font-mono text-xs text-ink/45">#{{ $d->id }} · {{ $d->slug }}</p>
                        <p class="mt-1 line-clamp-2 text-xs text-ink/55">{{ \Illuminate\Support\Str::limit(strip_tags((string) $d->description), 120) }}</p>
                        <form method="POST" action="{{ route('admin.destinations.toggle', $d) }}" class="mt-3" title="{{ __('Toggle visibility on public site') }}">
                            @csrf
                            @method('PATCH')
                            <button type="submit" class="w-full rounded-lg border border-secondary/60 bg-surface/50 py-2 text-xs font-semibold text-primary transition hover:border-primary/35 hover:bg-primary/5">
                                {{ $d->is_active ? __('Turn off') : __('Turn on') }}
                            </button>
                        </form>
                    </div>
                    <div class="flex flex-wrap gap-2 border-t border-secondary/30 bg-surface/30 px-3 py-2">
                        <a href="{{ route('admin.destinations.edit', $d) }}" class="text-xs font-semibold text-primary hover:underline">{{ __('Edit') }}</a>
                        <span class="text-ink/25" aria-hidden="true">|</span>
                        @if($d->is_active)
                            <a href="{{ route('explore-africa.show', $d) }}" target="_blank" rel="noopener noreferrer" class="text-xs font-medium text-ink/70 hover:text-primary hover:underline">{{ __('View') }}</a>
                            <span class="text-ink/25" aria-hidden="true">|</span>
                        @endif
                        <form action="{{ route('admin.destinations.destroy', $d) }}" method="post" class="inline" onsubmit="return confirm(@json(__('Delete?')));">
                            @csrf
                            @method('DELETE')
                ...
                <div class="col-span-full rounded-2xl border border-dashed border-secondary/70 bg-white px-8 py-16 text-center shadow-inner">
                    <div class="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-primary/10 text-primary">
                        <svg class="h-7 w-7" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" aria-hidden="true">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 11.953 0 0112 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0121 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0112 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 013 12c0-1.606.547-3.086 1.407-4.261" />
                        </svg>
                    </div>
                    @if(filled($q) && $stats['total'] > 0)
                        <p class="mt-4 font-medium text-ink">{{ __('No destinations match your search') }}</p>
                        <p class="mx-auto mt-2 max-w-md text-sm text-ink/65">{{ __('Try another term or clear the search.') }}</p>
                        <a href="{{ route('admin.destinations.index') }}" class="mt-6 inline-flex rounded-xl border border-secondary/60 bg-white px-5 py-2.5 text-sm font-semibold text-ink shadow-sm hover:bg-surface">{{ __('Clear search') }}</a>
                    @else
                        <p class="mt-4 font-medium text-ink">{{ __('No destinations yet') }}</p>
                        <p class="mx-auto mt-2 max-w-md text-sm text-ink/65">{{ __('Add a region with photo, description, and sort order for the public site.') }}</p>
                        <a href="{{ route('admin.destinations.create') }}" class="mt-6 inline-flex rounded-xl bg-primary px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-primary/90">{{ __('Add your first destination') }}</a>
                    @endif
                </div>
            @endforelse
        </div>

        @if($destinations->isNotEmpty() && $destinations->hasPages())
            <div class="flex flex-col gap-3 border-t border-secondary/40 pt-6 sm:flex-row sm:items-center sm:justify-between">
                <p class="text-sm text-ink/60">
                    {{ __('Showing :from–:to of :total', ['from' => $destinations->firstItem(), 'to' => $destinations->lastItem(), 'total' => $destinations->total()]) }}
                </p>
