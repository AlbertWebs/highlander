...
<div
    ...
    x-data="galleryDropzone({ uploadUrl: @js($uploadUrl), maxFiles: 30 })"
>
    <h2 class="text-base font-semibold text-ink">{{ __('Bulk upload (dropzone)') }}</h2>
    <p class="mt-1 text-sm text-ink/60">{{ __('Drag images here or click to select. Set a default category, then adjust per row before uploading.') }}</p>
    ...
    <div class="mt-6 flex flex-wrap items-center gap-3" x-show="rows.length > 0" x-cloak>
        <button
            ...
            @click="submit()"
            :disabled="uploading"
            class="inline-flex items-center justify-center rounded-xl bg-primary px-6 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-60"
        >
