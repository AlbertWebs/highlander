...
@push('meta')
    ...
    <meta property="og:image" content="{{ $destination->cardImageUrl() }}">
    <meta name="twitter:card" content="summary_large_image">
@endpush

@php
    $heroImage = $destination->cardImageUrl();
@endphp

@section('content')
@include('partials.page-hero', [
    'title' => $destination->name,
    'subtitle' => __('Where the journey begins.'),
    'image' => $heroImage,
    'kicker' => __('Explore Africa'),
    'immersive' => true,
    'wide' => true,
])

<section class="relative z-10 mt-6 bg-surface pb-20 sm:mt-8 lg:mt-10" aria-labelledby="destination-overview-heading">
        ...
        <div class="w-full rounded-t-[1.25rem] border border-secondary/25 bg-gradient-to-b from-white to-surface/90 px-5 py-10 shadow-[0_-8px_40px_rgba(46,46,46,0.07)] sm:rounded-t-[1.75rem] sm:px-10 sm:py-12 lg:px-14">
            <div class="flex flex-col gap-4 border-b border-secondary/25 pb-8 sm:flex-row sm:items-center sm:justify-between sm:gap-6">
                <nav class="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs font-medium text-ink/55 sm:text-sm" aria-label="{{ __('Breadcrumb') }}">
                    ...
                    <a href="{{ route('explore-africa') }}" class="rounded-md text-primary transition hover:bg-primary/5 hover:text-primary hover:underline">{{ __('Explore Africa') }}</a>
                    <span class="text-ink/30" aria-hidden="true">
                        <svg class="inline h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                    </span>
                    <span class="max-w-[min(100%,20rem)] truncate text-ink/80 sm:max-w-[min(100%,28rem)]" title="{{ $destination->name }}">{{ $destination->name }}</span>
                </nav>
                <a
                    href="{{ route('explore-africa') }}"
            ...
            <div class="mt-10 lg:grid lg:grid-cols-12 lg:items-stretch lg:gap-10 xl:gap-14 2xl:gap-16">
                <div class="min-w-0 lg:col-span-8">
                    <div class="flex flex-wrap items-end justify-between gap-3 border-b border-secondary/20 pb-4">
                        <div>
                            <h2 id="destination-overview-heading" class="text-[0.65rem] font-semibold uppercase tracking-[0.22em] text-primary">{{ __('Overview') }}</h2>
                            <p class="mt-1 text-sm text-ink/55">{{ __('Landscape, wildlife, and trip ideas for this region.') }}</p>
                        </div>
                        <a href="#plan-this-destination" class="inline-flex shrink-0 text-sm font-semibold text-primary underline decoration-primary/35 underline-offset-2 transition hover:decoration-primary">
                            {{ __('Jump to planning') }}
                        </a>
                    </div>
            ...
            <div id="plan-this-destination" class="mt-14 scroll-mt-28 sm:mt-16">
                <div class="rounded-card border border-secondary/40 bg-gradient-to-br from-white via-surface/80 to-tint-green/25 px-6 py-9 shadow-card sm:px-10 sm:py-11">
                    ...
                    <h2 class="mt-2 text-center font-serif text-xl font-semibold text-ink sm:text-2xl">{{ __('Plan or ask about this destination') }}</h2>
                    <p class="mx-auto mt-2 max-w-lg text-center text-sm leading-relaxed text-ink/60">{{ __('We will match dates, interests, and pacing so the trip feels right for you.') }}</p>
                    <div class="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-4">
                        <a
                            href="{{ route('plan-my-safari', ['destination' => $destination->slug]) }}"
