...
<section class="relative z-10 mt-6 bg-surface pb-20 sm:mt-8 lg:mt-10" aria-labelledby="safari-experience-overview-heading">
    <div class="site-gutter-x w-full max-w-none">
        <div class="w-full rounded-t-[1.25rem] border border-secondary/25 bg-gradient-to-b from-white to-surface/90 px-5 py-10 shadow-[0_-8px_40px_rgba(46,46,46,0.07)] sm:rounded-t-[1.75rem] sm:px-10 sm:py-12 lg:px-14">
            ...
            <div class="mt-10 lg:grid lg:grid-cols-12 lg:items-stretch lg:gap-10 xl:gap-14 2xl:gap-16">
                <div class="min-w-0 lg:col-span-8">
                    <div class="flex flex-wrap items-end justify-between gap-3 border-b border-secondary/20 pb-4">
                        <div>
                            <h2 id="safari-experience-overview-heading" class="text-[0.65rem] font-semibold uppercase tracking-[0.22em] text-primary">{{ __('Overview') }}</h2>
                            <p class="mt-1 text-sm text-ink/55">{{ __('What this style includes and how we can tailor it.') }}</p>
                        </div>
                        <a href="#plan-this-safari-style" class="inline-flex shrink-0 text-sm font-semibold text-primary underline decoration-primary/35 underline-offset-2 transition hover:decoration-primary">
                            {{ __('Jump to planning') }}
                        </a>
                    </div>
                    ...
                    @else
                        <div class="mt-6 rounded-card border border-dashed border-secondary/50 bg-surface/80 p-8 text-center">
                            <p class="text-lg leading-relaxed text-ink/65">{{ __('We are preparing more detail for this style. Use the planner below and we will tailor dates, parks, and pacing for you.') }}</p>
                        </div>
                    @endif
                </div>
            ...
            <div id="plan-this-safari-style" class="mt-14 scroll-mt-28 sm:mt-16">
                <div class="rounded-card border border-secondary/40 bg-gradient-to-br from-white via-surface/80 to-tint-green/25 px-6 py-9 shadow-card sm:px-10 sm:py-11">
                    ...
                    <h2 class="mt-2 text-center font-serif text-xl font-semibold text-ink sm:text-2xl">{{ __('Plan or ask about this safari style') }}</h2>
                    <p class="mx-auto mt-2 max-w-lg text-center text-sm leading-relaxed text-ink/60">{{ __('We will match reserves, season, and add-ons so the trip feels right for you.') }}</p>
                    <div class="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-4">
                        <a
                            href="{{ route('plan-my-safari', ['safari' => $safariExperience->slug]) }}"
