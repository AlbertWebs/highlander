...
<div class="space-y-6 lg:sticky lg:top-24 lg:z-10 lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto lg:overscroll-y-contain lg:pr-1 xl:top-28 xl:max-h-[calc(100vh-7rem)]">
    <div class="overflow-hidden rounded-2xl border border-primary/20 bg-gradient-to-br from-primary/[0.08] via-white to-tint-green/35 shadow-card ring-1 ring-primary/10" aria-labelledby="destination-glance-heading">
        <div class="border-b border-primary/10 bg-primary/[0.06] px-5 py-4">
            <div class="flex items-start gap-3">
                ...
                <div>
                    <p id="destination-glance-heading" class="text-[0.65rem] font-semibold uppercase tracking-[0.2em] text-primary">{{ __('At a glance') }}</p>
                    <p class="mt-0.5 text-xs text-ink/55">{{ __('Destination') }}</p>
                </div>
            </div>
        </div>
        <div class="px-5 py-5">
    ...
    <div class="rounded-2xl border border-secondary/40 bg-gradient-to-br from-surface to-white p-4 shadow-sm sm:p-5">
        ...
        <ul class="mt-3 space-y-2">
            <li>
                <a href="{{ route('plan-my-safari', ['destination' => $destination->slug]) }}" class="flex items-center justify-between gap-3 rounded-xl border border-transparent bg-white px-3 py-2.5 text-sm font-medium text-ink shadow-sm ring-1 ring-secondary/40 transition hover:border-primary/25 hover:ring-primary/20">
            ...
            <li>
                <a href="{{ route('explore-africa') }}" class="flex items-center justify-between gap-3 rounded-xl border border-transparent bg-white px-3 py-2.5 text-sm font-medium text-ink shadow-sm ring-1 ring-secondary/40 transition hover:border-primary/25 hover:ring-primary/20">
                    <span class="flex items-center gap-2">
                        <svg class="h-4 w-4 shrink-0 text-primary" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/></svg>
                        {{ __('All destinations') }}
                    </span>
                    <svg class="h-4 w-4 shrink-0 text-ink/35" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
                </a>
