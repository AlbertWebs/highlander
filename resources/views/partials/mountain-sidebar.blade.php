...
<div class="space-y-6 lg:sticky lg:top-24 lg:z-10 lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto lg:overscroll-y-contain lg:pr-1 xl:top-28 xl:max-h-[calc(100vh-7rem)]">
            ...
            <div class="border-b border-primary/10 bg-primary/[0.06] px-5 py-4">
                <div class="flex items-start gap-3">
                    <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white shadow-sm ring-1 ring-primary/15" aria-hidden="true">
                        <svg class="h-5 w-5 text-primary" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 18L9 11.25l2.25 3 3.75-4.5L21.75 18M4.5 5.25h15M3.75 21h16.5"/></svg>
                    </span>
                    <div>
                        <p id="elevation-aside-heading" class="text-[0.65rem] font-semibold uppercase tracking-[0.2em] text-primary">{{ __('At a glance') }}</p>
                        <p class="mt-0.5 text-xs text-ink/55">{{ __('Summit elevation') }}</p>
                    </div>
                </div>
            </div>
            <div class="px-5 py-5">
                <p class="font-serif text-4xl font-semibold tabular-nums tracking-tight text-ink sm:text-[2.75rem]">
                    {{ number_format($mountain->elevation_m) }}<sup class="ml-0.5 text-[0.45em] font-semibold text-ink/45">m</sup>
                </p>
                <p class="mt-4 border-t border-secondary/35 pt-4 text-sm leading-relaxed text-ink/65">
                    {{ __('Heights are approximate; conditions on the mountain vary by season. We plan routes and pacing for your group.') }}
                </p>
            </div>
        </div>
    @endif
        ...
        <div class="border-b border-secondary/30 bg-gradient-to-r from-primary/[0.06] to-transparent px-4 py-3.5 sm:px-5">
            <div class="flex items-center gap-2">
                ...
                <div>
                    ...
                    <p class="text-xs text-ink/50">{{ __('Tours & itineraries that pair well with this area') }}</p>
                </div>
            </div>
        </div>
    ...
    <div class="rounded-2xl border border-secondary/40 bg-gradient-to-br from-surface to-white p-4 shadow-sm sm:p-5">
        ...
        <ul class="mt-3 space-y-2">
            <li>
                <a href="{{ route('plan-my-safari', ['mountain' => $mountain->slug]) }}" class="flex items-center justify-between gap-3 rounded-xl border border-transparent bg-white px-3 py-2.5 text-sm font-medium text-ink shadow-sm ring-1 ring-secondary/40 transition hover:border-primary/25 hover:ring-primary/20">
